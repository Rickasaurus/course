...
(<*>) ::
  ...
  -> f b
(<*>) fab fa = (<$> fa) =<< fab
-- (<$> fa) Partial apply to the right gives: (a -> b) -> fb
-- this slots into =<< : (a -> fb) -> f a -> f b
--    as ((a -> b) -> f b) -> f (a -> b) -> f b
--    combined : f (a -> b) -> f b
-- the type of fab is f (a -> b) so this slots in and we get f b
