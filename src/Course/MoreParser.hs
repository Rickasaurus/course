...
-- flbindParser pa $ \val ->
--         flbindParser spaces $ \_ -> valueParser val
...
string ::
  Chars
  -> Parser Chars
string chrs = traverse (\c -> is c) chrs
...
stringTok ::
  Chars
  -> Parser Chars
stringTok chrs = tok $ string chrs
...
oneof ::
  Chars
  -> Parser Char
oneof chrs = satisfy (\c -> elem c chrs)
...
noneof ::
  Chars
  -> Parser Char
noneof chrs = satisfy (\c -> notElem c chrs)
...
hex ::
  Parser Char
hex = do v <- replicateA 4 (satisfy isHexDigit)
         res <- case readHex v of
                Full hv -> pure hv
                Empty -> failed
         pure $ chr res
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 parseVal parseSep =
  do v <- parseVal
     res <- list $ do parseSep
                      parseVal
     pure $ v :. res
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby parseVal parseSep = sepby1 parseVal parseSep ||| valueParser Nil
...
eof ::
  Parser ()
eof = P (\v -> if isEmpty v then Result Nil () else ErrorResult Failed)
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll fnLst = satisfy $ and . sequence fnLst

--  do v <- character
--                      if and $ map (\f -> f v) fnLst
--                      then valueParser v
--                      else failed
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny fnLst = satisfy $ or . sequence fnLst
